...
public abstract class GeneratorContext implements IGeneratorContext {
	...
	protected static void createInstance(GeneratorContextConfig config) {
		...
		try {
			Class<IGeneratorContext> clazz = (Class<IGeneratorContext>) Class.forName(implClass);
			instance = (IGeneratorContext)clazz.newInstance();
		} catch (ClassNotFoundException e) {
			throw new RuntimeException("GeneratorContext.implClass class not found!", e);
		} catch (InstantiationException e) {
			throw new RuntimeException("GeneratorContext.implClass class instantiation error!", e);
	...
	public static IGeneratorContext getInstance() {
		if (threadLocalInstance.get() == null) {
			throw new RuntimeException("GeneratorContext instance not ready! Method GeneratorContext.createInstance not invoked!");
		}
	...
	private GeneratorContextConfig configuration = null;
	...
	public void addConfiguration(GeneratorContextConfig newConfiguration) {
		if(this.configuration == null){
			...
			this.configuration = merger.merge(this.configuration, newConfiguration);
		}
	}
	
	public GeneratorContextConfig getConfiguration() {
		if(configuration == null){
			throw new RuntimeException("GeneratorContext.configuration not loaded!");
		}
	...
	public String getProperty(String path) {
		...
		try {
			...
			//throw new RuntimeException("GeneratorContext.getPropertyAsString error accessing property: " + path);
		}
	...
	public void run(String group, List<EntityInfo> entityList) throws Exception {
		...
		if(groupConfig != null){
			...
			for(EntityInfo entityInfo : entityList){
				...
				for(ReferenceGenerator referenceGenerator :  groupConfig.getGenerator()){
	...
	protected void processGenerator(IGenerator generator, EntityInfo entityInfo) throws Exception {
	...
	protected IGenerator getGenerator(GeneratorConfig generatorConfig) {
		...
		if(generator == null){
			...
			try {
				...
				throw new RuntimeException("Error instantiating the Generator. clazz: " + clazz, e);
			} catch (IllegalAccessException e) {
				throw new RuntimeException("Error instantiating the Generator. clazz: " + clazz, e);
			} catch (ClassNotFoundException e) {
	...
	protected void setupGenerator(IGenerator generator, GeneratorConfig generatorConfig) {
	...
	public IHelper getHelperById(String id) {
		...
		if(helper == null){
			...
			try {
				helper = (IHelper)loadClass(helperConfig.getClazz()).newInstance();
			} catch (InstantiationException e) {
				throw new RuntimeException("Error instantiating the Helper. clazz: " + helperConfig.getClazz(), e);
			} catch (IllegalAccessException e) {
				throw new RuntimeException("Error instantiating the Helper. clazz: " + helperConfig.getClazz(), e);
			} catch (ClassNotFoundException e) {
				throw new RuntimeException("Class not found for Helper. clazz: " + helperConfig.getClazz(), e);
			}
	 ...
	 * @return
	 */
	private GeneratorConfig getOneToOneGeneratorConfig(ReferenceGenerator referenceGenerator) {
		for(GeneratorConfig generatorConfig : configuration.getOneToOne().getGenerator()){
	 ...
	 * @return
	 */
	private GeneratorConfig getManyToOneGeneratorConfig(ReferenceGenerator referenceGenerator) {
		for(GeneratorConfig generatorConfig : configuration.getManyToOne().getGenerator()){
			if(generatorConfig.getId().equals( referenceGenerator.getRef() )){
				return generatorConfig;
			}
		}
